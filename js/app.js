...
function seedBoardDummyData(){
  try{
    const seededKey = "codein_board_seeded_v1";
    ...
    const now = Date.now();
    const dummy = [
      { id:"p_"+(now-500000), board:"free", title:"[공지] 첫 인사 & 사용법", content:"자유게시판에서는 스터디 모집/잡담/정보 공유가 가능해요.\n회원은 글 작성 가능!", author:"admin1", createdAt:(now-500000) },
      { id:"p_"+(now-400000), board:"free", title:"스터디 같이 하실 분?", content:"CS/네트워크 같이 공부하실 분 구합니다!\n댓글 기능은 추후 추가 예정.", author:"member1", createdAt:(now-400000) },
      { id:"p_"+(now-300000), board:"qa", title:"AWS EC2 요금이 갑자기 늘었어요", content:"Cost Explorer로 원인 추적하는 방법 추천 부탁드립니다.", author:"member1", createdAt:(now-300000) },
      { id:"p_"+(now-200000), board:"qa", title:"NCP Cloud DB for MySQL 접속 오류", content:"접속 도메인/포트/보안그룹 확인 포인트가 뭐가 있을까요?", author:"admin1", createdAt:(now-200000) }
    ];

    localStorage.setItem(BOARD_STORE_KEY, JSON.stringify(dummy));
    localStorage.setItem(seededKey, "1");
  }catch(e){
    // ignore
  }
...
const board = (qs.get("board") || "").toLowerCase();
const posts = loadBoardPosts()
  .filter(p => !board || p.board === board)
...
function initBoardsCrud(){
  ...
  // Gate: 비회원이면 보이게, 회원이면 숨김
  if(gate) gate.style.display = canWrite ? "none" : "";

  // 폼은 무조건 숨김 시작 (작성 버튼 눌러야만 열림)
  if(form) form.style.display = "none";

  // 작성 버튼: 회원만 가능
  if(openBtn){
    openBtn.style.display = canWrite ? "" : "none";
    if(!openBtn.dataset.bound){
      openBtn.dataset.bound = "1";
      openBtn.addEventListener("click", ()=>{
        if(!hasRole(ROLES.MEMBER)) return alert("회원만 작성할 수 있어요.");
  ...
  if(cancelBtn && !cancelBtn.dataset.bound){
    ...
    cancelBtn.addEventListener("click", ()=>{
      ...
      if(gate) gate.style.display = hasRole(ROLES.MEMBER) ? "none" : "";
    });
  }
  ...
  if(form && !form.dataset.bound){
    ...
    form.addEventListener("submit", (e)=>{
      e.preventDefault();
      if(!hasRole(ROLES.MEMBER)) return;
      ...
      const qs = new URLSearchParams(location.search);
      const board = (qs.get("board") || "free").toLowerCase();

      const posts = loadBoardPosts();
      posts.push({ id:"p_"+Date.now(), board, title:t, content:c, author:auth.username||"member", createdAt:Date.now() });
  ...
  if(list && !list.dataset.bound){
    ...
    list.addEventListener("click", (e)=>{
      const btn = e.target.closest("[data-del],[data-action=delete-post]");
      if(!btn) return;
      if(!hasRole(ROLES.ADMIN)) return;

      const id = btn.getAttribute("data-del") || btn.getAttribute("data-id");
      const next = loadBoardPosts().filter(p => p.id !== id);
      saveBoardPosts(next);
      renderBoardPosts();
    });
  }
...
const customNotices = loadCustomNoticesRaw().map(normalizeNotice);
  ...
  const stub = document.getElementById("boardsStub") || document.querySelector("#boardsStub");
  if(!stub) return;

  const qs = new URLSearchParams(location.search);
  const board = (qs.get("board") || "").toLowerCase(); // "", "free", "qa"

  // 1) 게시판 선택 화면
  if(board !== "free" && board !== "qa"){
    stub.innerHTML = `
      <div class="card card--padded">
        <h2 class="card__title">게시판</h2>
        <p class="muted">게시판을 선택해 주세요.</p>
      </div>

      <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:14px; margin-top:16px;">
        <a class="card card--padded" href="./boards.html?board=free" style="text-decoration:none; color:inherit;">
          <div style="font-weight:900; font-size:18px;">자유게시판</div>
          <div class="muted" style="margin-top:8px;">정보 공유, 스터디 모집, 잡담.</div>
          <div class="muted" style="margin-top:10px; font-size:12px;">→ 글 보러가기</div>
        </a>

        <a class="card card--padded" href="./boards.html?board=qa" style="text-decoration:none; color:inherit;">
          <div style="font-weight:900; font-size:18px;">Q&amp;A</div>
          <div class="muted" style="margin-top:8px;">개발/과제/인프라 질문을 모아 해결.</div>
          <div class="muted" style="margin-top:10px; font-size:12px;">→ 질문 보러가기</div>
        </a>
      </div>

      <div class="card card--padded" style="margin-top:16px;">
        <div style="display:flex; align-items:center; justify-content:space-between; gap:12px;">
          <div style="font-weight:900; font-size:16px;">최근 글</div>
          <div class="muted" style="font-size:12px;">더미 데이터 포함</div>
        </div>
        <div id="boardsRecentList" style="margin-top:12px; display:grid; gap:10px;"></div>
      </div>
    `;
    // 최근 글 렌더
    try{
      const el = document.getElementById("boardsRecentList");
      if(el){
        const posts = loadBoardPosts().slice().sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)).slice(0,4);
        if(posts.length === 0){
          el.innerHTML = `<div class="muted">아직 글이 없습니다.</div>`;
        }else{
          el.innerHTML = posts.map(p=>{
            const b = p.board === "qa" ? "Q&A" : "자유";
            const href = `./boards.html?board=${p.board}`;
            ...
            return `<a href="${href}" style="text-decoration:none; color:inherit;">
              <div class="card card--padded" style="padding:12px 14px;">
                <div style="display:flex; gap:10px; align-items:center; justify-content:space-between;">
                  <div style="font-weight:800;">[${b}] ${escapeHtml(p.title||"")}</div>
                  <div class="muted" style="font-size:12px;">${escapeHtml(dt)}</div>
                </div>
                <div class="muted" style="margin-top:6px; font-size:12px;">${escapeHtml(p.author||"")}</div>
              </div>
            </a>`;
          }).join("");
        }
  ...
  // 2) 게시판 상세 화면(글 목록/작성)
  const title = board === "qa" ? "Q&A" : "자유게시판";
  stub.innerHTML = `
    <div class="card card--padded">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:12px;">
        <div>
          <h2 class="card__title" style="margin:0;">${title}</h2>
          <p class="muted" style="margin-top:8px;">회원은 작성 가능, 운영자(ADMIN) 이상은 삭제 가능합니다.</p>
        </div>
        <a class="btn btn--ghost" href="./boards.html">목록으로</a>
      </div>
    </div>

    <div id="boardWriteGate" class="card card--padded" style="margin-top:16px;">
      <div style="font-weight:800; font-size:16px;">글 작성은 로그인 후 가능합니다.</div>
      <p class="muted" style="margin-top:8px;">회원(MEMBER)부터 작성할 수 있어요.</p>
      <a class="btn" href="./login.html?next=boards.html?board=${board}" style="margin-top:10px; display:inline-flex;">로그인 하러가기</a>
    </div>

    <div class="card card--padded" style="margin-top:16px;">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:12px;">
        <div style="font-weight:900; font-size:16px;">글 목록</div>
        <button id="openWriteBtn" class="btn" type="button">작성</button>
      </div>
      <p class="muted" style="margin-top:8px;">작성 버튼은 회원부터 사용 가능합니다.</p>
    </div>

    <form id="boardWriteForm" class="card card--padded" style="margin-top:16px; display:none;">
      <div style="font-weight:900; font-size:16px;">새 글 작성</div>
      <div style="margin-top:12px; display:grid; gap:10px;">
        <input name="title" class="input" placeholder="제목" maxlength="80" />
        <textarea name="content" class="input" placeholder="내용" rows="6"></textarea>
        <div style="display:flex; gap:10px; justify-content:flex-end;">
          <button id="cancelWriteBtn" class="btn btn--ghost" type="button">취소</button>
          <button class="btn" type="submit">등록</button>
        </div>
      </div>
    </form>

    <div id="boardPostList" style="margin-top:16px; display:grid; gap:12px;"></div>
  `;
}
...
function initCalendarAdd() {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (p === "boards") { seedBoardDummyData(); renderBoards(); renderGuestGate(); initBoardsCrud(); }
    if (p === "calendar") {
      initCalendarAdd();
      renderCalendar();
    }
